	...
	<div :class="[prefixCls + '-wrap', { disabled }]" ref="slider" :style="wrapStyle" @click.self="sliderClick">
		...
		<template v-if="showDots">
			<template v-if="!vertical">
				<div :class="[{active: item.active}, prefixCls + '-dot']" v-for="(item, index) in dots" :key="index" :style="{ 'left': item.val + '%' }" @click.self="sliderClick"></div>
			</template>

			<template v-else>
				<div :class="[{active: item.active}, prefixCls + '-dot-vertical']" v-for="(item, index) in dots" :key="index" :style="{ 'bottom': item.val + '%' }" @click.self="sliderClick"></div>
			</template>
		</template>

		<!-- 刻度 -->
		<div :class="[prefixCls + '-scales']" v-show="showScales" :style="scaleWrapStyle">
			<span v-for="(item, index) in scales" :key="index" :style="item">{{item.val}}</span>
		</div>

		<!-- 色块 -->
		<div :class="[prefixCls + '-button-wrap']" :style="minbtnStyle" @touchstart="onHandStart($event, 'min')" @mousedown="onHandStart($event, 'min')">
			<VTooltip :controlled="pointerDown === 'min'" :content="formatTip(exportValue[0])" :disabled="tipDisabled" :always="showTip === 'always'" :placement="tooltipPlace" ref="minTooltip">
				<div :class="minButtonClasses"></div>
			</VTooltip>
		</div>

		<div :class="[prefixCls + '-button-wrap']" :style="maxbtnStyle" v-show="range" @touchstart="onHandStart($event, 'max')" @mousedown="onHandStart($event, 'max')">
			<VTooltip :controlled="pointerDown === 'max'" :content="formatTip(exportValue[1])" :disabled="tipDisabled" :always="showTip === 'always'" :placement="tooltipPlace" ref="maxTooltip">
...
//					style.top = "-6px";
		...
		methods: {
			...
			getYDots() {
				...
				for(let i = 1; i < dotCount; i++) {
					const barStyle = {
						height: this.barStyle.height && Number(this.barStyle.height.slice(0, -1)),
						bottom:
